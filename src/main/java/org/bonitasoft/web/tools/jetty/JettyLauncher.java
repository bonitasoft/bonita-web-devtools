...
public class JettyLauncher extends ServletContainerLauncher {
    ...
    public static class JettyRequestLogger extends AbstractLifeCycle implements RequestLog {
        ...
        public JettyRequestLogger(final TreeLogger logger, final TreeLogger.Type normalLogLevel) {
            this.logger = logger;
        ...
        public void log(final Request request, final Response response) {
            ...
            if (logger.isLoggable(logStatus)) {
                final TreeLogger branch = logger.branch(logStatus, String.valueOf(status) + " - " + request.getMethod() + ' ' + request.getUri() + " ("
        ...
        private void logHeaders(final TreeLogger logger, final TreeLogger.Type logLevel, final HttpFields fields) {
            for (int i = 0; i < fields.size(); ++i) {
                final Field field = fields.getField(i);
     ...
     * 
     * The weird static data / default construction setup is a game we play with {@link Log}'s static initializer to prevent the initial log message from
     * going to stderr.
     */
    public static class JettyTreeLogger implements Logger {
        ...
        public JettyTreeLogger(final TreeLogger logger) {
            if (logger == null) {
        ...
        @Override
        public void info(final Throwable th) {
        ...
        @Override
        public void ignore(final Throwable th) {
    ...
    protected static class JettyServletContainer extends ServletContainer {
        ...
        public JettyServletContainer(final TreeLogger logger, final Server server, final WebAppContext wac, final int actualPort, final File appRootDir) {
            this.logger = logger;
            this.server = server;
            this.wac = wac;
        ...
        public void refresh() throws UnableToCompleteException {
            final String msg = "Reloading web app to reflect changes in " + appRootDir.getAbsolutePath();
            final TreeLogger branch = logger.branch(TreeLogger.INFO, msg);
        ...
        public void stop() throws UnableToCompleteException {
            final TreeLogger branch = logger.branch(TreeLogger.INFO, "Stopping Jetty server");
            // Temporarily log Jetty on the branch.
            Log.setLog(new JettyTreeLogger(branch));
            try {
                server.stop();
     ...
     * will not create a brand new {@link ClassLoader}. By creating a new {@code ClassLoader} each time, we re-read updated classes from disk.
     * 
    ...
    protected static final class WebAppContextWithReload extends WebAppContext {
        ...
        private class WebAppClassLoaderExtension extends WebAppClassLoader {
            ...
            public URL findResource(final String name) {
                ...
                final String warnMessage = "Server resource '" + name + "' could not be found in the web app, but was found on the system classpath";
            ...
            protected Class<?> findClass(final String name) throws ClassNotFoundException {
                ...
                final URL found = systemClassLoader.getResource(resourceName);
                if (found == null) {
                    return null;
                }

                // Warn, add containing URL to our own ClassLoader, and retry the call.
                final String warnMessage = "Server class '" + name + "' could not be found in the web app, but was found on the system classpath";
            ...
            private boolean addContainingClassPathEntry(final String warnMessage, final URL resource, final String resourceName) {
                final TreeLogger.Type logLevel = (System.getProperty(PROPERTY_NOWARN_WEBAPP_CLASSPATH) == null) ? TreeLogger.WARN : TreeLogger.DEBUG;
                TreeLogger branch = logger.branch(logLevel, warnMessage);
                String classPathURL;
                final String foundStr = resource.toExternalForm();
                ...
                branch = branch.branch(logLevel, "Adding classpath entry '" + classPathURL + "' to the web app classpath for this session", null,
        ...
        @SuppressWarnings("unchecked")
        private WebAppContextWithReload(final TreeLogger logger, final String webApp, final String contextPath) {
        ...
        public boolean isSystemClass(final String name) {
            return super.isSystemClass(name) || name.startsWith("org.apache.jasper.") || name.startsWith("org.apache.xerces.");
        }
        ...
        protected void doStop() throws Exception {
            ...
            final Class<?> jdbcUnloader = getClassLoader().loadClass("com.google.gwt.dev.shell.jetty.JDBCUnloader");
            final java.lang.reflect.Method unload = jdbcUnloader.getMethod("unload");
            unload.invoke(null);
    ...
    static {
         ...
         */
        final String antJavaC = System.getProperty("build.compiler", JDTCompiler16.class.getName());
     ...
     * @param port
     */
    private static void setupConnector(final AbstractConnector connector, final String bindAddress, final int port) {
        if (bindAddress != null) {
    ...
    @Override
    public boolean processArguments(final TreeLogger logger, final String arguments) {
        if (arguments != null && arguments.length() > 0) {
            ...
            for (final String arg : arguments.split(",")) {
                ...
                if ("ssl".equals(tag)) {
                    useSsl = true;
                    final URL keyStoreUrl = getClass().getResource("localhost.keystore");
                    if (keyStoreUrl == null) {
     ...
     */
    public void setBaseRequestLogLevel(final TreeLogger.Type baseLogLevel) {
    ...
    @Override
    public ServletContainer start(final TreeLogger logger, final int port, final File appRootDir) throws Exception {
        final TreeLogger branch = logger.branch(TreeLogger.TRACE, "Starting Jetty on port " + port, null);
        ...
        final AbstractConnector connector = getConnector(logger);
        setupConnector(connector, bindAddress, port);
        server.addConnector(connector);

        // Create a new web app in the war directory.
        final WebAppContext wac = createWebAppContext(logger, appRootDir);
        ...
        configureServer(server, appRootDir);
        ...
        final int connectorPort = connector.getLocalPort();
    ...
    protected JettyServletContainer createServletContainer(final TreeLogger logger, final File appRootDir, final Server server, final WebAppContext wac,
            final int localPort) {
    ...
    protected AbstractConnector getConnector(final TreeLogger logger) {
        if (useSsl) {
            final TreeLogger sslLogger = logger.branch(TreeLogger.INFO, "Listening for SSL connections");
            if (sslLogger.isLoggable(TreeLogger.TRACE)) {
                sslLogger.log(TreeLogger.TRACE, "Using keystore " + keyStore);
            }
            final SslSocketConnector conn = new SslSocketConnector();
            if (clientAuth != null) {
                switch (clientAuth) {
                    case NONE:
    ...
    private void checkStartParams(final TreeLogger logger, final int port, final File appRootDir) {
        if (logger == null) {
     ...
     * (http://www.apache.org/).
     */
    private void jreLeakPrevention(final TreeLogger logger) {
        ...
        try {
            ...
            final Method method = clazz.getDeclaredMethod("requestLatency", new Class[] { long.class });
        ...
        try {
            ...
            final Method method = policyClass.getMethod("getPolicy");
            method.invoke(null);
        } catch (final ClassNotFoundException e) {
